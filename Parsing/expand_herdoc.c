...
/*   Updated: 2024/12/11 15:47:29 by abait-ou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//     token->cmd[0] = '\0';
// }
static void noquotes(char *line, t_envvar *env)
{
    char *new_string;
    int new_len = ft_strlen(line) * 100;  // Allocate enough space
...
static void ft_replace(char *line, t_envvar *env)
{
    
    if (ft_quotes(line, indexdol(line)))
    {
        if (isinornot(line) == 2)
        {
            noquotes(line, env);
